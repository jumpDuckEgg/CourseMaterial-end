    ...
    <el-form ref="form" :model="form" :rules="rules" label-width="120px" @submit.native.prevent>
      ...
      <el-form-item label="课程图片上传:" required>
        <el-upload name='file' :action="imageUploadUrl" :data="{
                token
              }" ref="logoUpload" :limit='limitNum' :on-exceed='handleExceed' :before-upload="beforeImageUpload" list-type="picture-card" :on-preview="handlePictureCardPreview" :on-progress='handleProgress' :on-success="handleSuccess" :on-remove="handleRemove">
      ...
      <el-form-item label="课程申报书:" required>
        <el-upload name='file' drag :data="{
                'token':docToken
              }" ref="docUpload" :action="docUploadUrl" :before-upload="beforeDocUpload" :limit="limitNum" :on-exceed='handleExceed' :on-success="handleDocSuccess" :on-progress="handleProgress" :on-remove="handleDocRemove">
