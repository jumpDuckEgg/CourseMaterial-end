<template>
  <div class="examineCourse" v-loading="loading" element-loading-text="拼命加载中" element-loading-spinner="el-icon-loading">
    ...
    <div style="margin:10px 0;text-align:right">
      <el-input style="width:200px" placeholder="请输入课程名称或者作者" v-model="keyword"></el-input>
      <el-button type="primary" icon="el-icon-search" @click="sreachCourse">搜索</el-button>
    </div>
    <div>
      <el-table :data="currentData" style="width:100%;margin:10px 0;" border>
...
export default {
    ...
    methods: {
        sreachCourse() {
            ...
            this.sreachCourses = _.filter(this.content, function(o) {
                return rule.test(o.course_name) || rule.test(o.author);
            });

            this.total = this.sreachCourses.length;
            let offset = 0;
            this.currentData =
                offset + this.pageSize >= this.sreachCourses.length
                    ? this.sreachCourses.slice(
                          offset,
                          this.sreachCourses.length
                      )
                    : this.sreachCourses.slice(offset, offset + this.pageSize);
            this.currentPage = 1;
        },
