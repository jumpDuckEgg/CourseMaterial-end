    ...
    <div>
      <el-table :data="content" style="width:100%">
       ...
       <el-table-column label="操作">
        <template slot-scope="scope">
          <el-button
            size="mini" :disabled="scope.row.isPublish=='examine'"
            @click="handleEdit(scope.$index, scope.row)">{{scope.row.isPublish=="fail"?'重新上传':'编辑'}}</el-button>
          <el-button
            size="mini"
            type="danger" :disabled="scope.row.isPublish=='examine'"
...
export default {
  ...
  methods: {
    ...
    handleEdit(index, row) {
      ...
      this.docfileList = [{ name: "课程申请书", url: row.course_declaration }];
    },
